...
def update_product(row):
    ...
    payload = {"number": row.get("number"), "sales_price": row.get("dkk")}
    headers = {
...
with open(csv_file, "r", newline="") as file:
    ...
    for row in csv_reader:
        ...
        if row.get("number"):
            ...
            time.sleep(
                0.125
            )  # Pause for 0.125 seconds (1/8th of a second) between requests
        else:
            print("Skipping row: Number is required")
            print("-----------------------------------")

print("")
print("Update of product sales_price complete!")
